...
const fisik = {
  ...
  "2026-09-03": { [IDs.ungkep]:31, [IDs.taliwang]:13, [IDs.sopAyam]:9, [IDs.telor]:143, [IDs.sateKambing]:78, [IDs.sateAyam]:220, [IDs.sopIga]:11 },
...
const missingMovements = [
  // Sept 2 Masuk
  { id: "MOV-FIX-2-1", type: "Production", ingredient_id: IDs.taliwang, ingredient_name: "Ayam Taliwang (sub)", qty: 10, unit: "pcs", date: "2026-09-02", time: "10.00", note: "Recovered unrecorded production" },
  { id: "MOV-FIX-2-2", type: "Production", ingredient_id: IDs.sateAyam, ingredient_name: "Sate Ayam (sub)", qty: 192, unit: "pcs", date: "2026-09-02", time: "10.00", note: "Recovered unrecorded production" },
  { id: "MOV-FIX-2-3", type: "Production", ingredient_id: IDs.sopAyam, ingredient_name: "Sop Ayam (sub)", qty: 8, unit: "portion", date: "2026-09-02", time: "10.00", note: "Recovered unrecorded production" },
  // Sept 3 Waste
  { id: "MOV-FIX-3-1", type: "Waste", ingredient_id: IDs.ungkep, ingredient_name: "Ayam Bumbu Kuning (sub)", qty: -1, unit: "pcs", date: "2026-09-03", time: "22.00", note: "Spoiled/Waste (Reported by User)" },
  { id: "MOV-FIX-3-2", type: "Waste", ingredient_id: IDs.taliwang, ingredient_name: "Ayam Taliwang (sub)", qty: -1, unit: "pcs", date: "2026-09-03", time: "22.00", note: "Spoiled/Waste (Reported by User)" },
  // Sept 4 Masuk
  { id: "MOV-FIX-4-1", type: "Production", ingredient_id: IDs.sateKambing, ingredient_name: "Sate Kambing (sub)", qty: 305, unit: "pcs", date: "2026-09-04", time: "10.00", note: "Meldy production (Recovered)" },
];
...
async function run() {
  ...
  // 1. Delete and insert missing movements
  ...
  console.log("Injected missing movements (Masuk and Waste)");

  // 2. Clear previous recon
  ...
  for (const date of ["2026-09-02", "2026-09-03", "2026-09-04"]) {
    ...
    // Get true movements (Masuk & Waste)
    ...
    const payload = {
      ...
      data: { date: date, staff_name: 'Randy', notes: `Backdated with TRUE POS Math (${date})`, total_variance_value: totalVar, items: itemsPayload }
    };
  ...
  // Update live stock
  for (const [id, qty] of Object.entries(fisik["2026-09-04"])) {
    await supabase.from('ingredients').update({ stock: qty }).eq('id', id);
  }
  console.log("Live stock synced to Sept 4 closing balances");
}
run();
